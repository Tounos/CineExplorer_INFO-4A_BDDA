...
mkdir -p "$BASE_DIR/data/mongo/bd-1"
mkdir -p "$BASE_DIR/data/mongo/bd-2"
mkdir -p "$BASE_DIR/data/mongo/bd-3"
mkdir -p "$BASE_DIR/data/mongo/logs"
echo "Répertoires bd-1, bd-2 et bd-3 créés"
...
mongod --replSet rs0 --port 27017 --dbpath "$BASE_DIR/data/mongo/bd-1" --bind_ip localhost --logpath "$BASE_DIR/data/mongo/logs/db-1.log" > /dev/null 2>&1 &

# bd-2 (port 27018)
echo "Démarrage de bd-2 (port 27018)..."
mongod --replSet rs0 --port 27018 --dbpath "$BASE_DIR/data/mongo/bd-2" --bind_ip localhost --logpath "$BASE_DIR/data/mongo/logs/db-2.log" > /dev/null 2>&1 &

# bd-3 (port 27019)
echo "Démarrage de bd-3 (port 27019)..."
mongod --replSet rs0 --port 27019 --dbpath "$BASE_DIR/data/mongo/bd-3" --bind_ip localhost --logpath "$BASE_DIR/data/mongo/logs/db-3.log" > /dev/null 2>&1 &
...
echo ""
echo "=== Vérification que les instances répondent ==="
for port in 27017 27018 27019; do
    echo -n "Test port $port... "
    if mongosh --port $port --eval "db.adminCommand('ping')" --quiet > /dev/null 2>&1; then
        echo "✓ OK"
    else
        echo "✗ ERREUR - L'instance ne répond pas"
        exit 1
    fi
done
...
echo "Attente de l'élection du PRIMARY (30 secondes)..."
sleep 30

echo ""
echo "=== Vérification que le PRIMARY est élu ==="
mongosh --port 27017 --eval "rs.isMaster().ismaster && print('PRIMARY prêt') || print('En attente...')"
...
# Connexion directe au PRIMARY (plus fiable que via replica set)
mongorestore --host localhost:27017 /tmp/mongodb_backup
